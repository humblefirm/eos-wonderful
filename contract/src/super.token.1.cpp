...
    //코인 전송(키->계정)(sa)
    ...
    //토큰 전송 시그니쳐
    //코인 전송 시그니쳐
    //계정 생성 시그니쳐 (시그니쳐 생성을 하나로 통일)

    //새로운 이오스원더풀 기능, V3
    //테이블 3개, 
    //(usrbalance)(키)[해시드키인덱스,키,넌스,밸런스,이오스],
    //(namebalance)(계정)[네임인덱스,밸런스,이오스],
    //(info)(계정)[설정자 계정명, 계정 생성 비-토큰, 계정 생성 비-코인]
    //액션 4개
    //계정 생성(키|계정), (코인|토큰 지불, 지불한 금액은 지정된 관리자 계좌로. 일정 금액 이상 설정 불가.)
    //토큰 발행(키|계정),  (계정인 경우 영수증 발행) (무료 계정 생성,오직 컨트랙 자신 혹은 설정자만 가능)
    //토큰 전송(키|계정->키|계정), (계정인 경우 영수증 발행) 
    //코인 전송(키|계정->키|계정)
    //주요 기능
    //키|계정 string으로 받은 뒤 key인지 name인지 판단, 키(1), 계정(0), 에러(-1)
    //string to key
    //string to name
    //buffer to sig (통일된 시그니쳐 생성기), (고난이도)
